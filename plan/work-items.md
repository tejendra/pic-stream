...
#### [DONE] 9.1 Define jobs collection and create job on finalize
...
- [x] Job doc is created with status "pending" for each video in finalize
- [x] Collection and field names match

#### [DONE] 9.2 Implement video processing worker
...
- [x] Media doc gets previewPath and thumbnailPath and job is "done", or job is "failed" and paths stay null
- [x] Temp files are removed

#### [DONE] 9.3 Secure ffmpeg invocation
...
- [x] ffmpeg is invoked with array args; no user-controlled string in shell
- [x] ffmpeg is present in container
...
_Not implemented: Feature 9 uses the Firestore `jobs` worker (9.2); 9.4 remains an optional alternate design._
